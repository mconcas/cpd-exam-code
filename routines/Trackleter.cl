...
__kernel void Trackleter(
    ...
    const int first_cluster0 = lut0[group_id * kNz];
    const int last_cluster0  = lut0[(group_id + 1) * kNz];

    const int first_trkl = n_tracklets(lut0,lut1,group_id);
    const int n_trkl = get_nclusters(lut1,group_id + 1) + get_nclusters(lut1,group_id) + get_nclusters(lut1,group_id - 1);
    ...
    for (int iC0 = first_cluster0 + local_id; iC0 < last_cluster0; iC0 += group_size) {
        ...
        for (int j = group_id - 1; j <= group_id + 1; ++j) {
