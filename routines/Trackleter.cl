...
inline int get_nclusters(__global int* lut, int iPhi) {
    iPhi &= (kNphi - 1);
    return lut[(iPhi + 1) * kNz] - lut[iPhi * kNz];
};

inline int n_tracklets(__global int* lut0, __global int* lut1, int nphi) {
    int n = 0;
...
__kernel void Trackleter(
    ...
    const int first_trkl = n_tracklets(lut0,lut1,lid);
